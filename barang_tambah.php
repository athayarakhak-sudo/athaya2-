...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        // 1) Cek dulu apakah PHP sendiri gagal menerima file (sebelum disentuh kode kita)
        $uploadErrorMessages = [
        ...
        if ($_FILES['foto']['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!in_array($ext, $allowed)) {
                ...
                // 2) Baru di sini kita cek permission folder tujuan
                ...
                if (!move_uploaded_file($_FILES['foto']['tmp_name'], UPLOAD_DIR . $fotoNama)) {
                    ...
                    $error = 'Gagal mengunggah foto ke server. Detail: ' . ($lastErr['message'] ?? 'tidak diketahui');
                    $fotoNama = null;
                }
            }
        }
    ...
    if ($error === '') {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (str_contains($dbErr, 'Duplicate')) {
                ...
                $error = 'Gagal menyimpan data. Detail: ' . $dbErr;
            } else {
